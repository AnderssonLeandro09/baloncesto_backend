...
class AdministradorServiceTests(SimpleTestCase):
    ...
    def test_create_requires_email_password(self):
        ...
        with self.assertRaises(ValidationError):
            self.service.create_administrador(
                {"first_name": "A", "password": "pwd"}, {"cargo": "Admin"}, "token"
            )
        # Missing password
        with self.assertRaises(ValidationError):
            self.service.create_administrador(
                {"first_name": "A", "email": "a@b.com"}, {"cargo": "Admin"}, "token"
            )
    ...
    def test_create_success(self):
        ...
        admin_obj = SimpleNamespace(
            id=1, persona_external="abc", cargo="Admin", estado=True
        )
        ...
        self.service.dao.create.assert_called_with(
            persona_external="abc", cargo="Admin", estado=True
        )
    ...
    def test_create_recovers_from_duplicate_error(self):
        ...
        self.service.dao.exists.return_value = False
        admin_obj = SimpleNamespace(
            id=1, persona_external="existing_ext", cargo="Admin", estado=True
        )
        self.service.dao.create.return_value = admin_obj
        ...
        self.service._call_user_module.assert_any_call(
            "get", "/api/person/search_identification/123", "token"
        )
    ...
    def test_update_success_with_new_external(self):
        admin_obj = SimpleNamespace(
            id=1, persona_external="old", cargo="Admin", estado=True
        )
        ...
        updated_obj = SimpleNamespace(
            id=1, persona_external="new", cargo="SuperAdmin", estado=True
        )
        self.service.dao.update.return_value = updated_obj
        self.service.dao.exists.return_value = False

        result = self.service.update_administrador(
            1,
