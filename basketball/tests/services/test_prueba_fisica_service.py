...
class PruebaFisicaServiceTest(TestCase):
    ...
    def test_get_all_pruebas_fisicas_completas(self):
        ...
        from unittest.mock import patch
        with patch.object(self.service, '_fetch_persona', return_value={"data": {"first_name": "Ext", "last_name": "User", "identification": "000"}}):
            # Simulamos un usuario con rol adecuado
            mock_user = type('obj', (object,), {'role': 'ESTUDIANTE_VINCULACION', 'pk': '1'})
            results = self.service.get_all_pruebas_fisicas_completas("fake-token", user=mock_user)
    ...
    def test_get_atletas_habilitados_con_persona(self):
        ...
        with patch.object(self.service, '_fetch_persona', return_value=None):
            mock_user = type('obj', (object,), {'role': 'ESTUDIANTE_VINCULACION', 'pk': '1'})
            atletas = self.service.get_atletas_habilitados_con_persona("fake-token", user=mock_user)
            ...
            self.assertEqual(atleta_info["persona"]["identificacion"], self.atleta.cedula or "N/A")
