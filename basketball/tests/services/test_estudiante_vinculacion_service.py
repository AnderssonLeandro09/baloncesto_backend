...
class EstudianteVinculacionServiceTests(SimpleTestCase):
    ...
    def test_create_requires_persona(self):
        with self.assertRaises(ValidationError):
            self.service.create_estudiante({}, {'carrera': 'Ing', 'semestre': '1'}, 'token')
    ...
    def test_create_success(self):
        ...
        estudiante_obj = SimpleNamespace(id=1, persona_external='abc', carrera='Ing', semestre='1', eliminado=False)
        ...
        self.service.dao.create.assert_called_with(persona_external='abc', carrera='Ing', semestre='1', eliminado=False)
    ...
    def test_update_success_with_new_external(self):
        estudiante_obj = SimpleNamespace(id=1, persona_external='old', carrera='Ing', semestre='1', eliminado=False)
        ...
        updated_obj = SimpleNamespace(id=1, persona_external='new', carrera='Ing', semestre='2', eliminado=False)
        self.service.dao.update.return_value = updated_obj
        self.service.dao.exists.return_value = False

        result = self.service.update_estudiante(1, {'external': 'old', 'first_name': 'A', 'identification': '123'}, {'semestre': '2'}, 'token')
