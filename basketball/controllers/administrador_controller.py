...
class AdministradorController(viewsets.ViewSet):
    ...
    @extend_schema(responses={200: AdministradorResponseSerializer(many=True)})
    ...
    def create(self, request):
        ...
        administrador_data = (
            payload.get("administrador") or payload.get("administrador_data") or {}
        )
        try:
            result = self.service.create_administrador(
                persona_data or {}, administrador_data, token
            )
    ...
    @extend_schema(responses={200: AdministradorResponseSerializer})
    ...
    def update(self, request, pk=None):
        ...
        try:
            result = self.service.update_administrador(
                pk, persona_data or {}, administrador_data, token
            )
            if not result:
                return Response(
                    {"error": "Administrador no encontrado"},
