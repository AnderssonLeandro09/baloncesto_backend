...
class PruebaAntropometricaCreateDTO(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if altura_sentado > estatura:
            raise serializers.ValidationError("Altura sentado no puede ser mayor que estatura")

        if envergadura < estatura - 5:
            raise serializers.ValidationError("Envergadura debe ser al menos estatura - 5 cm")
        ...
        if rol == 'ENTRENADOR' and not entrenador:
            raise serializers.ValidationError("Debe especificar el entrenador registrador")
        if rol == 'ESTUDIANTE_VINCULACION' and not estudiante:
            raise serializers.ValidationError("Debe especificar el estudiante de vinculación registrador")
...
class PruebaAntropometricaResponseDTO(serializers.ModelSerializer):
    ...
    atleta_apellido = serializers.CharField(source='atleta.apellido_atleta', read_only=True)
    ...
    def get_registrado_por_nombre(self, obj):
        ...
        elif obj.rol_registrador == 'ESTUDIANTE_VINCULACION' and obj.registrado_por_estudiante:
