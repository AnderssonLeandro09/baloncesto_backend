...
class EstudianteVinculacionService:
    ...
    def _call_user_module(self, method: str, path: str, token: str, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        headers = self._build_auth_header(token)
        url = f"{self.user_module_url}{path}"
        try:
            response = requests.request(method=method, url=url, headers=headers, json=payload, timeout=8)
    ...
    def _search_by_identification(self, identification: Optional[str], token: str) -> Optional[Dict[str, Any]]:
        if not identification:
            return None
        try:
            return self._call_user_module('get', f"/api/person/search_identification/{identification}", token)
        except Exception:
            return None

    def _search_in_all_filter(self, identification: str, token: str) -> Optional[Dict[str, Any]]:
        try:
            response = self._call_user_module('get', '/api/person/all_filter', token)
            data = response.get('data')
            if isinstance(data, list):
                for person in data:
                    if isinstance(person, dict) and person.get('identification') == identification:
                        return person
            return None
        except Exception:
            return None
    ...
    def _fetch_persona(self, external: str, token: str, allow_fail: bool = False) -> Optional[Dict[str, Any]]:
        try:
            return self._call_user_module('get', f"/api/person/search/{external}", token)
        except Exception:
            if allow_fail:
                return None
            raise

    def _build_response(self, estudiante: EstudianteVinculacion, persona_payload: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # CRUD operations
    # ======================================================================
    def create_estudiante(self, persona_data: Dict[str, Any], estudiante_data: Dict[str, Any], token: str) -> Dict[str, Any]:
        if not persona_data:
        ...
        try:
            ...
            if not persona_external and persona_data.get('identification'):
                lookup_response = self._search_by_identification(persona_data.get('identification'), token)
            ...
            if 'ya esta registrada' in message.lower() or 'already registered' in message.lower():
                persona_response = self._search_by_identification(persona_data.get('identification'), token)
        ...
        if not persona_external:
            ...
            fallback_person = self._search_in_all_filter(persona_data.get('identification'), token)
            if fallback_person:
        ...
        if self.dao.exists(persona_external=persona_external, eliminado=False):
            raise ValidationError("Ya existe un estudiante de vinculación con ese external")
    ...
    def update_estudiante(self, pk: int, persona_data: Dict[str, Any], estudiante_data: Dict[str, Any], token: str) -> Optional[Dict[str, Any]]:
        ...
        if ident:
            ...
            new_external = self._extract_external(lookup_response) if lookup_response else None
            
        # Fallback si la búsqueda falló o no se proveyó identificación (poco probable)
        if not new_external:
             new_external = estudiante.persona_external

        if new_external != estudiante.persona_external and self.dao.exists(persona_external=new_external, eliminado=False):
            raise ValidationError("El external_id retornado ya está en uso por otro estudiante")
    ...
    def get_estudiante(self, pk: int, token: str) -> Optional[Dict[str, Any]]:
        ...
        if not estudiante or estudiante.eliminado:
            return None
        persona_info = self._fetch_persona(estudiante.persona_external, token, allow_fail=True)
        return self._build_response(estudiante, persona_info)
