...
class AdministradorService:
    ...
    def _build_response(self, administrador: Administrador, persona_payload: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # CRUD operations
    # ======================================================================
    def create_administrador(self, persona_data: Dict[str, Any], administrador_data: Dict[str, Any], token: str) -> Dict[str, Any]:
        if not persona_data:
    ...
    def update_administrador(self, pk: int, persona_data: Dict[str, Any], administrador_data: Dict[str, Any], token: str) -> Optional[Dict[str, Any]]:
        ...
        if new_external != administrador.persona_external and self.dao.exists(persona_external=new_external, estado=True):
            raise ValidationError("El external_id retornado ya está en uso por otro administrador")
    ...
    def get_administrador_by_id(self, pk: int, token: str) -> Optional[Dict[str, Any]]:
        ...
        if not administrador or not administrador.estado:
            return None
        persona_info = self._fetch_persona(administrador.persona_external, token, allow_fail=True)
        return self._build_response(administrador, persona_info)
    ...
    def get_all_administradores(self, token: str) -> List[Dict[str, Any]]:
        ...
        for admin in administradores:
            persona_info = self._fetch_persona(admin.persona_external, token, allow_fail=True)
