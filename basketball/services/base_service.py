...
class ServiceResult:
    ...
    def is_error(self) -> bool:
        ...
        return self.status in (ResultStatus.ERROR, ResultStatus.VALIDATION_ERROR, ResultStatus.CONFLICT)
    
    @classmethod
    def success(cls, data: Any = None, message: str = "Operación exitosa") -> "ServiceResult":
    ...
    @classmethod
    def validation_error(cls, message: str, errors: List[str] = None) -> "ServiceResult":
        """Crea un resultado de error de validación"""
        return cls(status=ResultStatus.VALIDATION_ERROR, message=message, errors=errors or [])
